...
**Suppression rules.** Where the org's workflows make certain default signal triggers noisy or wrong for them specifically, write explicit suppression rules rather than leaving this implicit in `<ignore>`. Suppression rules work best when they give Echo a concrete test to apply, not just a category to avoid. A strong suppression rule usually has three parts:
...
This pattern generalises well beyond any one org — e.g. suppressing signals from outbound-only comms unless there's a material inbound response, distinguishing routine support load from real escalations using an objective threshold (time open, explicit language, direct CSM contact), or discounting a customer-wide communication style (e.g. a naturally urgent-sounding industry or persona) that would otherwise read as risk. When the org's CS materials or support model imply this kind of noisy-default pattern, write it as its own `<critical_suppression_rules>` block within Prompt 3, ahead of `<churn_signals>`/`<upsell_signals>`, so Echo applies it before signal detection rather than as an afterthought.
...
- [ ] If the org's workflows make a default trigger noisy (outbound-only comms, routine support load, industry-driven urgency/tone), Prompt 3 includes a `<critical_suppression_rules>` block with a concrete trigger/condition/fallback test — not just a vague `<ignore>` note
